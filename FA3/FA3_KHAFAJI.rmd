...
Since we can view signal transmission as mutually exclusive, and the errors in transmission are within the probability of each transmission, we can look at the problem as the union of success in signal 1, and the error of signal 0, as such:
...
For b, we need to imagine a new venn diagram, in which sig1_total is a proper superset of sig1_success, so then:
...
**p(sig1_success | sig1_result) = p(sig1_total (intersection) sig1_success)/p(sig1_total)**
...
**(a)**
As the work of each is mutually exclusive, and so are their errors, we can simply add up the probability of their errors
...
```
As seen from the results, the event that, when there is an error, it is Amy's program, is the highest.
